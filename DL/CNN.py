...
def train_model(X_train, y_train, X_val, y_val, input_dim, attention_type=None, epochs=100, batch_size=32, learning_rate=0.0001):
...
def plot_results(y_true, y_pred, title):
    ...
    rmse = np.sqrt(mean_squared_error(y_true, y_pred)) * 10  # 转换为 g·kg⁻¹
    ...
    for file_path, dataset_name in file_paths:
        ...
        for target_column, y in y_dict.items():
            ...
            for attention_type in ["6 layers", 'SE', 'ECA', 'CBAM']:
                ...
                test_metrics = evaluate_model(model, X_val, y_val, title=f"{dataset_name} - {target_column} - Test - {attention_type}")
                results.append((dataset_name, target_column, f"CNN_{attention_type}", train_metrics, test_metrics))
